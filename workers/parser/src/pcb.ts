...
/** Best-effort Altium / text netlist importer into DesignSnapshot-like components */
export function parseAltiumLikeText(src: string): {
  components: Array<{
    refdes: string;
    value: string;
    footprint: string;
  ...
  // Accept simple CSV: Ref,Value,Footprint,MPN
  ...
  for (const line of lines) {
    ...
    if (!/^[A-Z]+\d+/i.test(refdes)) continue;
    components.push({
      refdes,
      value: parts[1] ?? "",
      footprint: parts[2] ?? "",
      mpn: parts[3] || undefined,
    });
  }
  return {
    components,
